...
/**
 * the regex such np as.
 */
public class RegexSuchNPAs implements Regex {
    private String regex;
    private Pattern pt;
    /**
     * the builder for the class.
     */
    public RegexSuchNPAs() {
        regex = "such <np>([^<>/])+</np> as <np>([^<>/])+</np>(( ,)? <np>([^<>/])+</np>)*(( ,)?"
                + " (or|and) <np>([^<>/])+</np>)?";
    ...
    public List<String> getHypernym(String sentence) {
        ...
        String hyperny = sentence.substring(9, length);
